...
public class GamificationForceClient {
    ...
    public func fetch<T: Decodable>(type: T.Type, with request: URLRequest, urlSession: URLSession = .shared) async throws -> T {
        ...
        do {
            ...
            let newRequest = GamificationForceRequest.setAuthorization(request: request, accessToken: token)
            ...
            let updatedRequest = GamificationForceRequest.setAuthorization(request: request, accessToken: token)
