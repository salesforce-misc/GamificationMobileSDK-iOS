...
public class NetworkManager: NetworkManagerProtocol {
    ...
    internal func handleDataAndResponse(output: (data: Data, response: URLResponse)) throws -> Data {
        guard let httpResponse = output.response as? HTTPURLResponse else {
            ...
            GamificationLogger.error(error.description)
            throw error
        }

        GamificationLogger.debug("HTTP response code is: \(httpResponse.statusCode)")
        ...
        if !(200..<300).contains(httpResponse.statusCode) {
            ...
            GamificationLogger.error(CommonError.responseUnsuccessful(message: errorMessage, displayMessage: displayMessage).description)
    ...
    public func fetch<T: Decodable>(type: T.Type, request: URLRequest, urlSession: URLSession = URLSession.shared) async throws -> T {
        ...
        do {
            ...
            GamificationLogger.error("NetworkManager fetch/decode error: \(error)")
            throw error
        }
    }
}
